// ==UserScript==
// @name           Prime Battle Tools
// @namespace      fluffy
// @include        http://*.kingdomsofcamelot.com/*main_src.php*
// @description    KOC Prime Battle Tools - Better Scouting and Attacking
// @require        http://sizzlemctwizzle.com/updater.php?id=89356&days=5
// ==/UserScript==

var Version = '20110818';

var Title = 'KOC Prime Battle Tools';
...
function btStartup (){
  ...
  if (metc.width==null || metc.width==0){
    btStartupTimer = setTimeout (btStartup, 1000);
    return;
  }
  unsafeWindow.btLoaded = Version;
...
//logit ('g_timeoff: '+ unsafeWindow.g_timeoff);
  ...
  var gmstyles = '\
    .btcastleButNon {background-image:url("'+ URL_CASTLE_BUT +'")}\
    .btcastleButSel {background-image:url("'+ URL_CASTLE_BUT_SEL +'")}\
    .btcastleBut {outline:0px; margin-left:0px; margin-right:0px; width:24px; height:26px; font-size:12px; font-weight:bold;}\
    .btcastleBut:hover {border-size:3px; border-color:#000;}\
  ';
 
  var styles = '.xtab {padding-right: 5px; border:none; background:none; white-space:nowrap;}\
    .xtabBR {padding-right: 5px; border:none; background:none;}\
    div.btDiv {background-color:#f0f0f0;}\
    table.btTab tr td {border:none; background:none; white-space:nowrap;}\
    table.btTabPad tr td {border:none; background:none; white-space:nowrap; padding: 2px 4px 2px 4px;}\
    table.btTabBR tr td {border:none; background:none;}\
    table.btTabLined tr td {border:1px none none solid none;}\
    table.btTabOverview tr td {border-left:1px solid #ccc; white-space:nowrap; padding: 1px;}\
    table.btTabPad tr td.btentry {background-color:#ffeecc; padding-left: 8px;}\
    table.btNoPad tr td {border:none; background:none; white-space:nowrap; padding:0px}\
    table.btPlayers tr td {background-color:none; padding-left:5px; padding-right:5px;}\
    .btOddrow {background-color:#eee}\
    .btstat {border:1px solid; border-color:#ffffff; font-weight:bold; padding-top:2px; padding-bottom:2px; text-align:center; color:#ffffff; background-color:#357}\
    .btStatLight {color:#ddd}\
    .btentry {padding: 7px; border:1px solid; border-color:#000000; background-color:#ffeecc; white-space:nowrap;}\
    .btErrText {font-weight:bold; color:#600000}\
    button::-moz-focus-inner, input[type="submit"]::-moz-focus-inner { border: none; }\
    .btChatWhisper {}\
    .btChatAlliance {}\
    .btChatGlobal {background-color: #fdd}\
    .btChatIcon {border: 3px inset red}\
    span.whiteOnRed {padding-left:3px; padding-right:3px; background-color:#700; color:white; font-weight:bold}\
    span.boldRed {color:#800; font-weight:bold}\
    span.boldDarkRed {color:#600; font-weight:bold}\
    a.btButton20 {color:#ffff80}\
    hr.btThin {padding:0px; margin:0px}\
    ...
    table.btMainTab {empty-cells:show; margin-top:5px }\
    table.btMainTab tr td a {color:inherit }\
    table.btMainTab tr td   {height:60%; empty-cells:show; padding: 0px 5px 0px 5px;  margin-top:5px; white-space:nowrap; border: 1px solid; border-style: none none solid none; }\
    table.btMainTab tr td.spacer {padding: 0px 4px;}\
    table.btMainTab tr td.sel    {font-weight:bold; font-size:13px; border: 1px solid; border-style: solid solid none solid; background-color:#eed;}\
    table.btMainTab tr td.notSel {font-weight:bold; font-size:13px; border: 1px solid; border-style: solid solid none solid; background-color:#1e66bd; color:white; border-color:black;}\
    tr.btPopTop td { background-color:#dde; border:none; height: 21px;  padding:0px; }\
    tr.btretry_btPopTop td { background-color:#a00; color:#fff; border:none; height: 21px; padding:0px; }\
    input.btButCancel {background-color:#a00; font-weight:bold; color:#fff}\
    .CPopup .CPopMain { background-color:#f8f8f8; padding:6px;}\
    .CPopup  {border:3px ridge #666}\
 	 .rightborder {border-left:0px;border-top:0px;border-bottom:0px;border-right:1px;border-style:solid;border-color:black}\
	 .topborder {border-left:0px;border-top:1px;border-bottom:0px;border-right:0px;border-style:solid;border-color:black}\
	 .toprightborder {border-left:0px;border-top:1px;border-bottom:0px;border-right:1px;border-style:solid;border-color:black})';
     
  logit ("* KOCprimeTools v"+ Version +" Loaded");
...
// TODO: Make sure WinPos is visible on-screen ?
  if (Options.btWinPos==null || Options.btWinPos.x==null|| Options.btWinPos.x=='' || isNaN(Options.btWinPos.x)){
//logit ( 'Options.btWinPos: '+ inspect (Options.btWinPos, 5, 1));
    var c = getClientCoords (document.getElementById('main_engagement_tabs'));
    Options.btWinPos.x = c.x+4;
    Options.btWinPos.y = c.y+c.height;
    saveOptions ();
  }
  mainPop = new CPopup ('btmain', Options.btWinPos.x, Options.btWinPos.y, 749,700, Options.btWinDrag,
      function (){
        tabManager.hideTab();
  ...
  AddMainTabLink('PRIME', eventHideShow, mouseMainTab);
TestSomething.init (); 
...
function btofficerId2String (oid){
  if (oid==null)
    return '';
  else if (oid==3)
    return 'Officer';
...
***/
btfetchPlayerCourt : function (uid, notify){
  var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
  params.pid = uid;
...
btfetchTEST : function (pageNum, notify){    // as in alliance list, sorted by rank, 10 per page
  var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
  params.pageNo = 1;
...
logit ("ajax/allianceGetMembersInfo.php:\n"+ inspect (rslt, 5, 1));     
  ...
  init : function (div){
    ...
    t.cont = div;
//t.fetchTEST();   
    unsafeWindow.BTgetMembers = t.bteventGetMembers;
    unsafeWindow.BTpd = t.btclickedPlayerDetail;
    unsafeWindow.BTpl = t.btclickedPlayerLeaderboard;
    if (btgetMyAlliance()[0] == 0) {
      t.cont.innerHTML = '<BR><BR><CENTER>You need to be in an alliance to use this feature.</center>';
      t.state = 1;
      return;
    }
    var m = '<DIV class=btentry >\
	 <TABLE width=100% border=0 cellpadding=0 cellspacing=0 class="xtab"><tr valign="top">\
	 <td style="background-color:#C0C0C0; width:240;height:100%">\
		<Table width="240" cellspacing=0 cellpadding=0 class="xtab" style="font-size:x-small;background-color:#C0C0C0;border: 1px solid black">\
			<tr><td align=left class="xtab">&nbsp;<u><b>Search Options:</b></u></td></tr>\
	      <TR><TD class=xtab left=right><b>&nbsp;Player Name:</b>&nbsp;</td></tr>\
   	      <tr><TD class=xtab>&nbsp;<INPUT id=btallPlayName type=text /> &nbsp; <INPUT id=btplaySubmit type=submit value="Player" /></td>\
      	   </tr>\
       	<TR><TD class=xtab align=left><b>&nbsp;Alliance name:</b> &nbsp;</td></tr>\
         	<tr><TD class=xtab>&nbsp;<INPUT id=btallAllName type=text /> &nbsp; <INPUT id=btallSubmit type=submit value="Alliance" /></td>\
          	</tr>\
				<tr><TD class="xtab btErrText"><SPAN id=btplayErr></span><SPAN id=btallErr></span>&nbsp;</td></tr>\
		</table>\
		</td>\
		<td style="background-color:#FFFFFF;width:475px;" >\
		<Table cellpadding=0 cellspacing=0 width=100% style="font-size:x-small;border: 1px solid black">\
			<tr><TD align="right" class="xtab"><div id="currScouts">\
				<table border=0 cellpadding=0 cellspacing=0 width=100%>\
					<tr><td class="xtab" style="width:350px">&nbsp;<b>Scout Options:</b><B><label id="lblMarches"></label></b></td></tr>\
					<tr><td>\
					<table width="100%" cellspacing=0 cellpadding=0 >\
						<tr style="border-bottom: 1px solid black"><td><center>Mode</center></td>\
						<td><center>Pattern</center></td>\
						<td><center>Type/Multi</center></td>\
						<td><center># Scouts</center></td></tr>\
						<tr valign="top" style="border-top:1px solid black">\
          				<td style="valign:middle;width:50px;border-right:1px #000000 solid" align="center" class="xtab" align=center><table border=0 cellpadding=0 cellspacing=0><tr style="height:8px"><td style="height:15px"><input style="height:10px" type="radio" id="rdo_ModeA" name="rdo_Mode" checked="yes" value="User" />Select</td></tr><tr><td><input style="height:15px" type="radio" id="rdo_ModeB" name="rdo_Mode" value="Offline"/>Offline</td></tr><tr><td><input style="height:10px" type="radio" id="rdo_ModeC" name="rdo_Mode" value="Online" />Online</td></tr></table></td>\
          				<td style="valign:middle;width:50px;border-right:1px #000000 solid" align="center" class="xtab" align=center><table border=0 cellpadding=0 cellspacing=0><tr style="height:15px"><td ><input type="radio" style="height:10px" id="rdo_PatternCity" name="rdo_Pattern" value="City" />City</td></tr><tr style="height:15px"><td><input type="radio" name="rdo_Pattern" id="rdo_PatternFastest" value="Fastest" checked="yes" />Fastest</td></tr><tr><td>Online Select<INPUT TYPE=CHECKBOX ID=Online NAME=Online></td></tr></table></td>\
          				<td style="valign:middle;width:50px;border-right:1px #000000 solid" align="center" class="xtab" align=center><table border=0 cellpadding=0 cellspacing=0><tr style="height:15px"><td ><input type="radio" style="height:10px" name="ScoutType" value="Scout" checked="yes" />Scout</td></tr><tr style="height:15px"><td><input type="radio" name="ScoutType" value="Attack" />Attack</td></tr><tr><td><select id=MarchMultiplier><option selected value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select></td></tr></table></td>\
          				<td class="xtab" align="center" valign="top" style="padding-top:5px;width:50px;">&nbsp;<select style="width:50px" id="scoutQty"><option selected value="1">1</option><option value="10">10</option><option value="50">50</option><option value="100">100</option><option value="500">500</option><option value="1000">1k</option><option value="5000">5k</option><option value="10000">10k</option><option value="25000">25k</option><option value="50000">50k</option><option value="90000">90k</option><option value="100000">100k</option><option value="150000">150k</option><option value="200000">200k</option></select><br/>&nbsp;<br/><INPUT id=ScoutSubmit type=submit value="Scout" /><INPUT id=ScoutHelp type=button value="Help" /></td></tr>\
          		</table>\
					</td></tr>\
				</table>\
          	</div></td></tr>\
...
</table><span style="vertical-align:middle;" id=btaltInput></span>\
</div><SPAN id=btallListOut></span>';
    t.cont.innerHTML = m;
	 document.getElementById('MarchMultiplier').addEventListener('change', t.btmarchMultiplierChange, false);
    document.getElementById('btallSubmit').addEventListener ('click', t.bteventSubmit, false);
    document.getElementById('btplaySubmit').addEventListener ('click', t.bteventPlayerSubmit, false);
    document.getElementById('btallAllName').addEventListener ('focus', function (){document.getElementById('btallErr').innerHTML='';}, false);
    document.getElementById('ScoutSubmit').addEventListener ('click', t.btsendScoutAttacksSubmit , false);
    document.getElementById('btallPlayName').addEventListener ('focus', function (){document.getElementById('btplayErr').innerHTML='';}, false);
	 document.getElementById('rdo_ModeA').addEventListener ('click', t.btAutoSelectCheckA , false);
	 document.getElementById('rdo_ModeB').addEventListener ('click', t.btAutoSelectCheckB , false);
	 document.getElementById('rdo_ModeC').addEventListener ('click', t.btAutoSelectCheckC , false);
  },

	btgetRallyLvl : function(cityId){
		var ret = 0;
        var b = Seed.buildings['city'+ cityId];
       	for(j=0; j<33; j++){
        	if(b['pos'+j][0] == 12)
         	   ret = parseInt(b['pos'+j][1]);
         }
		return ret;
	},
   btAutoSelectCheckA : function (){
       var t = Tabs.AllianceList;
		t.btclearChks();
		alert("User Select");	 
	},
   btAutoSelectCheckB : function (){
       var t = Tabs.AllianceList;
		t.btclearChks();
		alert("Offline Select");	 
	},
   btAutoSelectCheckC : function (){
      var t = Tabs.AllianceList;
		t.btclearChks();
		alert("Online Select");	 
	},

	// master control fuction
    btsendScoutAttacksSubmit : function (){
       var t = Tabs.AllianceList;
		if(t.btavailMarches < 0){
			alert('Too many targets selected.  Please uncheck targets until Available Marches is >= 0');
			return;
		}
		if (document.getElementById("rdo_PatternFastest").checked == true){
			// send from all cities.
			//alert("Whoot1");
			t.sendScoutMarchesFastest();
		}
		if (document.getElementById("rdo_PatternCity").checked == true){
			// send from specific city.
			alert("Whoot2");
		}
		alert ("Done");	
	},
	// end master control function
	
	// function that handles marches out to cities based on input arrays
	sendScoutMarchesFastest : function (){
		var t = Tabs.AllianceList;
		var cityMarchMaxArray = t.btfillCityMarchMaxArray();//created array to hold cities max marches (set by t.btfillCityMarchMaxArray.) (anything < 0 sets city max march = 0;)
		var cityCurrMarchCountArray = t.btfillCurrMarchCountArray();//inital set with t.btfillCurrMarchCountArray() (intialized to 0's)
      		var enemyCityArray = t.btsetEnemyCityArray()// cities to be scouted, set by checkboxes selected in form.
		var t = Tabs.AllianceList;
		var rankArray = [];var enemyCity = [];
		for (var i=0; i<enemyCityArray.length;i++){
			enemyCity = enemyCityArray[i];
			rankArray = t.btsetDistanceRankArray(enemyCity);
			for(var j=0; j<rankArray.length;j++){
				var currClosestCity = rankArray[j];
				var marchMultiplier = parseInt(document.getElementById("MarchMultiplier").selectedIndex) + 1;
				var marchMax = parseInt(cityMarchMaxArray[parseInt(currClosestCity)])
				var diffmarchMax_Curr = parseInt(cityMarchMaxArray[parseInt(currClosestCity)]) - (parseInt(cityMarchMaxArray[parseInt(currClosestCity)])- parseInt(cityCurrMarchCountArray[parseInt(currClosestCity)]));
				marchAvail = marchMax - diffmarchMax_Curr;
				if (parseInt(marchAvail) >= parseInt(marchMultiplier)){
					cityCurrMarchCountArray[currClosestCity] = parseInt(cityCurrMarchCountArray[currClosestCity]) + marchMultiplier;
					var marchCheck = parseInt(cityMarchMaxArray[parseInt(currClosestCity)]) - parseInt(cityCurrMarchCountArray[currClosestCity]);
					var ss = document.getElementById('scoutQty');
					var ssValue = ss.options[ss.selectedIndex].value;
					for (k=0; k<marchMultiplier;k++){
						if(parseInt(Seed.units['city'+Cities.cities[currClosestCity].id]['unt3']) >= ssValue && (t.btgetRallyLvl(Cities.cities[currClosestCity].id)*10000) >= ssValue ){
							t.btlaunchMarch(Cities.cities[currClosestCity].id, enemyCity[0], enemyCity[1], 0, ssValue);
						}
						else{
							alert('Too many scouts selected.  Change dropdown qty.');
							break;
						}
					}
					var sleepTime = Math.random()*5;
					if (sleepTime < 1.5) sleep(1500);
					else sleep(Math.round(sleepTime*1000,0));
					break;
				}
			}
		}
	...
	btclearChks : function(){
		document.getElementById('Online').checked = false;
		document.getElementById('selAllMarch').checked = false;
		var tbl = document.getElementById('allBody');
		for(i = 0; i<tbl.rows.length; i++){
			document.getElementById('selMarch_'+i).checked = false;
		}
	},
	
	// function to suck up checkboxed cities.
	btsetEnemyCityArray : function () {
		var enemyCityArray = [];
		var mytable = document.getElementById('allBody');
		for (var i=0; i <mytable.rows.length; i++){
        	var baseHolder = mytable.rows[i].cells[7].innerHTML;
           	var startPosition = baseHolder.indexOf('(');
           	var endPosition = baseHolder.indexOf(')');
           	var myCity = baseHolder.substr(startPosition, endPosition);
           	startPosition = myCity.indexOf('(')+1;
           	endPosition = myCity.indexOf(')')-1;
           	myCity = myCity.substr(startPosition, endPosition);
           	var cityXY = [];
		 	if(document.getElementById('selMarch_'+i).checked){
 	      		cityXY = myCity.split(",");
            	enemyCityArray.push(cityXY);
		   	}
		}
		return enemyCityArray;
	},
	
	// function to create corresponding rank array (rank array is array containing numers 0 < numCities and #s 
	// 	correspond to cityid's in order of closeness.
	btsetDistanceRankArray : function (enemyCity){
		var t = Tabs.AllianceList;
		var distarray = []; // array handling current city distance array
		var myCitiesRank = [];
		// get distances from cities to enemy city into distarray
		for (intI=0; intI<Cities.numCities; intI++){
			var mydistance = btdistance (enemyCity[0], enemyCity[1], Cities.cities[intI].x, Cities.cities[intI].y);
			distarray[intI] = parseFloat(mydistance);
			myCitiesRank[intI] = parseFloat(mydistance);
		}
		bubbleSort(myCitiesRank);
		var enemyCityAttackRank = []; 
		for (var i=0; i<myCitiesRank.length; i++){
			for (var j=0; j<distarray.length; j++){
				if (myCitiesRank[i] == distarray[j]){
					enemyCityAttackRank.push(j);
				}
			}
		}
		return enemyCityAttackRank;
	...
	// fill array with starting values of 0;
	btfillCurrMarchCountArray : function () {
		marchArray = [];
	...
	// fill marchmax array with max # of available marches for cities.
	btfillCityMarchMaxArray : function (){
		marchMaxArray = [];
		for (var i=0; i<Cities.numCities; i++){
      	var cityHolderID = Cities.cities[i].id;
         var marchMax=0;
         var b = Seed.buildings['city'+ Cities.cities[i].id];
         for(j=0; j<33; j++){
            if(b['pos'+j][0] == 12)
         	   marchMax = parseInt(b['pos'+j][1]);
         }
		for (x in Seed.queue_atkp) {
			var o = Seed.queue_atkp[x];
			if(x == 'city'+cityHolderID) marchMax = marchMax - parseInt(Object.keys(o).length);
		}
			marchMaxArray[i] = parseInt(marchMax/(document.getElementById("MarchMultiplier").selectedIndex + 1));
		}
		return marchMaxArray;
	},


	btlaunchMarch : function (cityId, x, y, knightId, scoutQty){
    	var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
       	var cityName = '';
       	params.gold = 0;
       	params.items = [];
       	...
       	new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, {
          	...
          	onSuccess: function (rslt) {
				...
				var ut = unixtime();
				attach_addoutgoingmarch(rslt.marchId, rslt.marchUnixTime, ut + timediff, params.xcoord, params.ycoord, 0, rslt.tileId, rslt.tileType, rslt.tileLevel, params.cid);
				var t = Tabs.AllianceList;
				t.btavailMarches = t.btavailMarches - 1;
				t.btupdateAvailMarchesLabel();
              	return true;
          	},
          	onFailure: function (rslt) {
              	return false;
	...
	btgetKnightIds : function(cityId){
		var did = {};
	   	var validKnights = [];
      	for (var i=0; i<knightRoles.length; i++){
          	var leader = Seed.leaders['city'+cityId][knightRoles[i][1]+'KnightId'];
         	if (leader == 0){}
         	else {
               	did['knt'+leader] = true;
         	}
      	...
      	for (k in Seed.knights['city'+cityId]){
           	if (!did[k]){
            	if(Seed.knights['city'+cityId][k].knightStatus != 10)
                	validKnights.push(Seed.knights['city'+cityId][k].knightId);
            }
        }
		return validKnights;
	},
	
	btmarchMultiplierChange : function(){
		var t = Tabs.AllianceList;
		t.btavailMarches = t.btgetTotalAvailMarches();
	  	t.btupdateAvailMarchesLabel();
	},
	
   btgetTotalAvailMarches : function(){
		var numMarches = 0;
   		for(i=0; i<Cities.numCities; i++){
			var b = Seed.buildings['city'+ Cities.cities[i].id];
            for(j=0; j<33; j++){
            	if(b['pos'+j][0] == 12)
                numMarches += parseInt(b['pos'+j][1]);
            }
		for (x in Seed.queue_atkp) {
			var o = Seed.queue_atkp[x];
			if(x == 'city'+Cities.cities[i].id) numMarches = numMarches - parseInt(Object.keys(o).length);
		}
        }
      	var tbl = document.getElementById('allBody');
		for(i = 0; i<tbl.rows.length; i++){
			if(document.getElementById('selMarch_'+i).checked){
				if(document.getElementById("MarchMultiplier").selectedIndex != 0){
					numMarches = numMarches - (document.getElementById("MarchMultiplier").selectedIndex + 1);
				}
				else{
					numMarches = numMarches - 1;
				}
			}
		}
		//alert(numMarches);
  ...
  btpName : '',
  bteventPlayerSubmit : function (){
    var t = Tabs.AllianceList;
    document.getElementById('btplayErr').innerHTML='';
    var name = document.getElementById('btallPlayName').value;
    t.btpName = name;
    if (name.length < 3){
      document.getElementById('btplayErr').innerHTML = 'Enter at least 3 characters';
      return;
    }
    document.getElementById('btaltInput').innerHTML = '';
    document.getElementById('btallListOut').innerHTML = '<BR><BR><CENTER>Searching ...</center>';
    t.btfetchPlayerList (name, t.bteventGotPlayerList);
  },
 
  bteventGotPlayerList : function (rslt){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
      document.getElementById('btallListOut').innerHTML = rslt.errorMsg;
      return;
    }
    t.btplayerList = rslt.matchedUsers;
    var uList = [];
    for (k in rslt.matchedUsers)
      uList.push (rslt.matchedUsers[k].userId);    
    t.btfetchPlayerStatus (uList, function(r){t.bteventGotPlayerOnlineList(r)});   
  },   
   
  bteventGotPlayerOnlineList : function (rslt){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
      document.getElementById('btallListOut').innerHTML = rslt.errorMsg;
      return;
    }
    var m = '<DIV class=btstat>Showing players matching <B>"'+ t.btpName +'"</b></div>\
      <DIV style="height:575px; max-height:575px; overflow-y:auto">\
      <TABLE width=100% align=center class=btTab cellspacing=0><TR style="font-weight:bold"><TD>Name</td>\
      <TD align=right>Might</td><TD> &nbsp; Online</td><TD> &nbsp;Facebook &nbsp; </td><TD width=75%>Lookup </td></tr>';
    var row=0;
    var cl='';
    for (k in t.btplayerList){
      var u = t.btplayerList[k];
      if (++row % 2)
        cl = 'class=btOddrow ';
      else
        cl = '';
      m += '<TR '+ cl +'valign=top><TD>'+ u.genderAndName +'</td><TD align=right>'+ btaddCommasInt(u.might) +'</td>\
          ...
          <TD><SPAN onclick="BTpd(this, '+ u.userId +')"><A>details</a> &nbsp; <BR></span><SPAN onclick="BTpl(this, \''+ u.userId +'\')"><A>leaderboard</a></span></td></tr>';
    }
    m += '</table></div>';
    document.getElementById('btallListOut').innerHTML = m;
  },
 
   
  btclickedPlayerDetail : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "fetching details ...";
    t.btfetchPlayerInfo (uid, function (r) {t.btgotPlayerDetail(r, span)});
  },

  btclickedPlayerLeaderboard : function (span, uid){
    var t = Tabs.AllianceList;
    span.onclick = '';
    span.innerHTML = "fetching leaderboard info ...";
    t.btfetchLeaderboard (uid, function (r) {t.btgotPlayerLeaderboard(r, span)});
 	 t.btupdateAvailMarches(0);

  },
 
  btgotPlayerLeaderboard : function (rslt, span){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
    ...
    if (!an || an=='' || p.officerType==4)
      ...
      an += ' ('+ btofficerId2String(p.officerType) +')';
    m = '<TABLE cellspacing=0 class=btTab><TR><TD><B>Leaderboard: </b></td><TD colspan=2> Might: '+ p.might  +' &nbsp; Alliance: '+ an +'</td></tr>';
    for (var i=0; i<p.cities.length; i++){
      ...
      +' (<a onclick="btGotoMap ('+ c.xCoord +',' +c.yCoord+ ')">'+ c.xCoord +','+ c.yCoord +'</a>)</td><TD width=75%> &nbsp; level: '
        + c.tileLevel +' &nbsp; status: '+ btcityStatusString (c.cityStatus) + created +'</td></tr>';
    } 
    span.innerHTML = m + '</table>';
  },
   
  btgotPlayerDetail : function (rslt, span){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
      span.innerHTML = rslt.errorMsg;
      return;
    ...
    var a = 'None';
    if (u.allianceName)
      a = u.allianceName +' ('+ btgetDiplomacy(u.allianceId) + ')';
    var m = '<TABLE cellspacing=0 class=btTab><TR><TD><B>Details:</b> &nbsp; </td><TD>Alliance: '+ a +' &nbsp; Cities: '
  ...
  bteventMyAllianceSubmit : function (){
    var t = Tabs.AllianceList;
    document.getElementById('btallListOut').innerHTML = '<BR><BR><CENTER>Searching ...</center>';
    t.btfetchAllianceMemberList (btgetMyAlliance()[0], null, t.bteventGotMemberList);
  }, 
   
  btaName : '',
  bteventSubmit : function (){
    var t = Tabs.AllianceList;
    document.getElementById('btallErr').innerHTML='';
    t.btaName = document.getElementById('btallAllName').value;
    if (t.btaName.length < 3){
      document.getElementById('btallErr').innerHTML = 'Enter at least 3 characters';
      return;
    }
    var myA = btgetMyAlliance ();
    document.getElementById('btaltInput').innerHTML = '';
    document.getElementById('btallListOut').innerHTML = '<BR><BR><CENTER>Searching ...</center>';
    if (myA[0]!=0 && myA[1].toLowerCase().indexOf(t.btaName.toLowerCase())>=0 )
      t.btfetchAllianceList (t.btaName, myA[0], t.bteventGotAllianceList);
    else
      t.btfetchAllianceList (t.btaName, null, t.bteventGotAllianceList);
  },

  bteventGotAllianceList : function (rslt){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
      document.getElementById('btallListOut').innerHTML = rslt.errorMsg;
      return;
    }
    var m = '<DIV class=btstat>Showing alliances matching <B>"'+ t.btaName +'"</b></div>\
    ...
    for (k in rslt.alliancesMatched){
      ...
      m += '<TR><TD class=xtab>'+ all.allianceName +'</td><TD align=right class=xtab>'+ all.ranking +'</td><TD align=right class=xtab>'+ all.membersCount +'</td>\
       <TD align=right class=xtab>'+ btaddCommasInt(all.might) +'</td><TD class=xtab>'+ dip +'</td>\
       <TD class=xtab><a onclick="BTgetMembers('+ all.allianceId +');">View Members</a></td></tr>';
    }
    document.getElementById('btallListOut').innerHTML = m;
  },


  bteventGotMemberList : function (rslt){
    var t = Tabs.AllianceList;
    if (!rslt.ok){
      document.getElementById('btallListOut').innerHTML = rslt.errorMsg;
      return;
    }
    t.btmemberListRslt = rslt;
    var uList = [];
    for (k in rslt.results)
      uList.push (rslt.results[k].userId);    
    t.btfetchPlayerStatus (uList, function(r){t.bteventGotMemberOnlineList(r)});   
  },   
   
  bteventGotMemberOnlineList : function (rslt){
    var t = Tabs.AllianceList;
    var numInvalid = 0;
    var numPlayers = 0;
    t.dat = [];
    for (var i=0; i<t.btmemberListRslt.results.length; i++){
      p = t.btmemberListRslt.results[i];
      if (p.userId == 0){
        ++numInvalid;
      } else {
        ++numPlayers;
    ...
    t.btsetDistances (Cities.cities[0].x, Cities.cities[0].y);
    t.btdisplayMembers (t.btmemberListRslt.allianceName, numPlayers);
  },

  // sort and display
  btreDisp : function (){
    var t = Tabs.AllianceList;
    function btsortFunc (a, b){
      var t = Tabs.AllianceList;
...
return 0;       
    ...
    t.dat.sort (btsortFunc);
    var m = '';
    for (var i=0; i<t.dat.length; i++){
      m += '<TR style="max-height:30px"><TD class=xxtab>'+ t.dat[i][0] +'</td><TD align=right class=xxtab>'+ btaddCommasInt(t.dat[i][1])
         +'</td><TD align=center class=xxtab>'+ t.dat[i][3] +'</td><TD class=xxtab>'+ btofficerId2String(t.dat[i][2])                      
         ...
         +'</td><TD align=center class=xxtab><DIV onclick="btGotoMap('+ t.dat[i][5] +','+ t.dat[i][6] +')"><A>'+ t.dat[i][5] +','+ t.dat[i][6] +'</a></div></td>\
            <TD align=right class=xxtab style="padding-right:20px;">'+ t.dat[i][8].toFixed(2) +'</td><td align=center><input type="checkbox" id="selMarch_'+i+'" onChange="btupdateAvailMarches(this.checked)" /></td></tr>';
    }
...
//new CtableToText('tabAllMembers').toText();
  ...
  btsetDistances : function (x, y){
    var t = Tabs.AllianceList;
    for (var i=0; i<t.dat.length; i++)
      t.dat[i][8] = btdistance (x, y, t.dat[i][5], t.dat[i][6]);
  },

  sortColNum : 8,
  sortDir : 1,

  btdisplayMembers : function (allName, numPlayers){
    var t = Tabs.AllianceList;
    function btalClickSort (e){
      var t = Tabs.AllianceList;
    ...
    unsafeWindow.BTalClickSort = btalClickSort;
    var m = '<STYLE>.clickable{background-color:#ddd; border:2px outset; border-color:#555; padding-left:5px; padding-right:5px}\
      ...
      <DIV class=btstat><TABLE id=tabAllMembers cellpadding=0  width=100%><TR font-weight:bold"><TD class=xtab> &nbsp; '+ allName +'</td>\
      ...
      <TR style="font-weight:bold"><TD id=clickCol0 onclick="BTalClickSort(this)" class=clickable><A><DIV>Player</div></a></td>\
        <TD id=clickCol1 onclick="BTalClickSort(this)" class=clickable align=center><A><DIV>Might</a></div></td>\
        <TD id=clickCol3 onclick="BTalClickSort(this)" class=clickable><A><DIV>Cities</a></div></td>\
        <TD id=clickCol2 onclick="BTalClickSort(this)" class=clickable align=center><A><DIV>Rank</a></div></td>\
        <TD id=clickCol9 onclick="BTalClickSort(this)" class=clickable align=center><A><DIV>Online</a></div></td>\
        <TD id=clickCol7 onclick="BTalClickSort(this)" class=clickable><A><DIV>City Name</a></div></td>\
        <TD id=clickCol4 onclick="BTalClickSort(this)" class=clickable><A><DIV>Lvl</a></div></td>\
        <TD id=clickCol5 onclick="BTalClickSort(this)" class=clickable><A><DIV>Coords</a></div></td>\
        <TD id=clickCol8 onclick="BTalClickSort(this)" class=clickable><A><DIV>Distance</a></div></td>\
        <TD id=clickCol9 class=clickable><DIV><input type="checkbox" id="selAllMarch" /></div></td></tr></thead>\
    ...
    document.getElementById('btallListOut').innerHTML = '<div style="overflow-y:auto; height:535px;">'+m+'</div>';
    document.getElementById('btaltInput').innerHTML = '<HR><TABLE width=100% cellpaddding=0><TR align=center>\
    ...
    t.btreDisp();
    new CdispCityPicker ('plyrdcp', document.getElementById ('dmcoords'), true, t.bteventCoords, 0).bindToXYboxes(document.getElementById('plyrX'), document.getElementById('plyrY'));
    document.getElementById('selAllMarch').addEventListener ('change', t.bteventSelAllMarch, false);
    document.getElementById('Online').addEventListener ('change', t.bteventSelOnlineMarch, false);
	t.btupdateAvailMarchesLabel();
	t.bteventSelOnlineMarch();
  },
    
  btupdateAvailMarchesLabel : function(){
  	var t = Tabs.AllianceList;
	document.getElementById('lblMarches').innerHTML = 'Available Marches = ' + t.btavailMarches +'&nbsp;&nbsp;';  
  },

  bteventSelAllMarch : function(){
	  var t = Tabs.AllianceList;
      var tbl = document.getElementById('allBody');
	  if(document.getElementById('selAllMarch').checked){
		  for(i = 0; i<tbl.rows.length; i++){
			  document.getElementById('selMarch_'+i).checked = true;
		  }
	  }
	  else{
		  for(i = 0; i<tbl.rows.length; i++){
  ...
  bteventSelOnlineMarch : function(){
	  var t = Tabs.AllianceList;	  
	  if(document.getElementById('allBody')){
	      var tbl = document.getElementById('allBody');
		  if(document.getElementById('Online').checked){
			  for(i = 0; i<tbl.rows.length; i++){
				  if(tbl.rows[i].cells[4].innerHTML == '<span class="boldDarkRed">ONLINE</span>')
			  		document.getElementById('selMarch_'+i).checked = true;
			  }
		  }
		  else{
  ...
  bteventCoords : function (city, x, y){
    var t = Tabs.AllianceList;
    var m = '';
    if (city != null)
    ...
    t.btsetDistances(x,y);
    t.btreDisp();
  },

  bteventGetMembers : function (aid){
    var t = Tabs.AllianceList;
    document.getElementById('btallListOut').innerHTML = '<BR><BR><CENTER>Searching ...</center>';
    t.btfetchAllianceMemberList (aid, null, t.bteventGotMemberList);
  },

  btfetchAllianceMemberList : function (allianceId, allianceName, notify) {
    var t = Tabs.AllianceList;
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    params.perPage = 100;
    if (allianceName)
  ...
  btfetchLeaderboard : function (uid, notify) {
    var t = Tabs.AllianceList;
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    params.userId = uid;
    new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/getUserLeaderboard.php" + unsafeWindow.g_ajaxsuffix, {
  ...
  btfetchAllianceList : function (allianceName, myAid, notify) {   // at least 3 chars :)
    var t = Tabs.AllianceList;
    function btcombineResults (rsltA, rsltM, notify){
      if (!rsltA.ok){
    ...
    new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/allianceGetSearchResults.php" + unsafeWindow.g_ajaxsuffix, {
      ...
      onSuccess: function (rslt) {
        if (myAid!=null && myAid>0)
          t.btfetchMyAllianceInfo  (function (r){ btcombineResults (rslt, r, notify)});
        else
          notify (rslt);
      },
      onFailure: function (rslt) {
  ...
  btfetchMyAllianceInfo : function (notify){
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
  ...
  btfetchPlayerList : function (name, notify){  // at least 3 chars!!
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
  ...
  btfetchPlayerInfo : function (uid, notify){
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
    params.uid = uid;
...
ajax/getOnline.php:
  ...
  (undefined) errorMsg: null = null
***/
  btfetchPlayerStatus : function (uidArray, notify){
    var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
...
unsafeWindow.btupdateAvailMarches = function (chkVal){
  var t = Tabs.AllianceList;
  if(chkVal){
	  if(document.getElementById("MarchMultiplier").selectedIndex != 0){
		  t.btavailMarches = t.btavailMarches - (document.getElementById("MarchMultiplier").selectedIndex + 1);
	  }
	  else{
		  t.btavailMarches = t.btavailMarches - 1;
		  //alert(t.btavailMarches);
	  }
	  //alert (t.btavailMarches);
	  t.btupdateAvailMarchesLabel();
  }
...
/*********************************** Battle TAB ***********************************/
unsafeWindow.btactivateCoords = function (rpt_id, x, y, defName, attName, rptTime){
    document.getElementById('xInput').value = x;
    document.getElementById('yInput').value = y;
    var t = Tabs.Battle;
    t.btgetMyReport(rpt_id, attName, defName, rptTime);
}

unsafeWindow.waveEnable = function(wave){
	for(i=1; i<13; i++){
		if(document.getElementById('wv'+wave+'chk').checked){
			document.getElementById('unit'+i+'wv1').disabled = false;
			document.getElementById('unit'+i+'wv1').style.backgroundColor = "#FFF";
			if(wave > 1){
				document.getElementById('unit'+i+'wv2').disabled = false;
				document.getElementById('unit'+i+'wv2').style.backgroundColor = "#FFF";
				document.getElementById('wv1chk').checked = true;
			}
			if(wave > 2){
				document.getElementById('unit'+i+'wv3').disabled = false;
				document.getElementById('unit'+i+'wv3').style.backgroundColor = "#FFF";
				document.getElementById('wv2chk').checked = true;
			}
		}
		else{
			document.getElementById('unit'+i+'wv3').disabled = true;
			document.getElementById('unit'+i+'wv3').style.backgroundColor = "#CCC";
			if(wave < 3){
				document.getElementById('unit'+i+'wv2').disabled = true;
				document.getElementById('unit'+i+'wv2').style.backgroundColor = "#CCC";
				document.getElementById('wv3chk').checked = false;
			}
			if(wave < 2){
				document.getElementById('unit'+i+'wv1').disabled = true;
				document.getElementById('unit'+i+'wv1').style.backgroundColor = "#CCC";
				document.getElementById('wv2chk').checked = false;
			}
		}
	}
}

unsafeWindow.btcalcTime = function(){
	var t = Tabs.Battle;
	for(wave=1; wave<4; wave++){
		if(document.getElementById('wv'+wave+'chk').checked){
			var a = {};
			for(i=1; i<13; i++){
				a[i] = document.getElementById('unit'+i+'wv'+wave).value;
			}
			document.getElementById('wv'+wave+'time').innerHTML = t.btmarchTimeCalculator(a);
		}
	}
}

unsafeWindow.btbattleGetAlliance = function (allianceId){
	var t = tabManager;
    newTab = t.tabList['AllianceList'];
   	t.setTabStyle (document.getElementById ('bttcBattle'), false);
    t.setTabStyle (document.getElementById ('bttc'+ newTab.name), true);
    ...
    newTab.obj.show();
    t = Tabs.AllianceList;
  	t.bteventGetMembers(parseInt(allianceId));
}  
Tabs.Battle = {
    tabOrder : 30,
    cont : null,
    ...
    init : function (div){
        ...
        t.cont = div;
        var    m = '<table>';
            m += '<tr><td  valign="top" colspan="6">';
             m += '<p><b>Enter coords to search:</b><br />x:<input size="3" type="text" id="xInput" value="0" onkeydown="btcalcTime()" onchange="btcalcTime()"/>  y:<input size="3" type="text" id="yInput" value="0" onkeydown="btcalcTime()" onchange=""btcalcTime()"/>';
            m += '&nbsp;&nbsp;<input type="button" id="sOn" value="Search" />&nbsp;&nbsp;<b>Report ID: </b><label id="searchMessage">0</label> &nbsp;<input id="scoutQtyB" size="5" value="1"> &nbsp;<input type="button" id="launchScout" value="Scout" onclick="launchScout()"/></p></td>';
            m += '<td><INPUT id=bcalcX type=hidden\><INPUT id=bcalcY type=hidden\><SPAN id=bbtloc1></span><DIV style="font-size:14px" id=bbtdistout>Distance:  &nbsp;<B>0</b></div><td></tr>';
            m += '<th style="font-size:14px" colspan="2">Attacker</th>';
            m += '<th style="font-size:14px" colspan="2">Defender</th>';
            m += '<th style="font-size:14px" colspan="2">Report Time</th>';
            m += '<th style="font-size:14px" colspan="2">Successful Scout</th>';
            m += '<tr><td align="center" colspan=2 style="background:none;border-bottom:none"><label id="attLabel"></label></td>';
            m += '<td align="center" colspan=2 style="background:none;border-bottom:none"><label id="defLabel"></label></td>';
            m += '<td align="center" width="230" colspan=2 style="background:none;border-bottom:none"><label id="timeLabel"></label></td>';
            m += '<td align="center" colspan=2 style="background:none;border-bottom:none"><label id="scoutLabel"></label></td></tr>';
            m += '<tr><td colspan="6" style="background:none;border-bottom:none">&nbsp;</td></tr>';
            m += '<th style="font-size:14px" colspan="2">Units</th>';
            m += '<th style="font-size:14px" colspan="2">Defenses</th>';
            m += '<th style="font-size:14px" colspan="3">Scouting Reports</th>';
            m += '<tr><td width="115" style="background:none;border-bottom:none"><b>Supply Troops:</b></td><td style="background:none;border-bottom:none" width="70"><label id="unit1">0</label></td>';
            m += '<td width="110" style="background:none;border-bottom:none"><b>Crossbows:</b></td><td style="background:none;border-bottom:none" width="70"><label id="def1">0</label></td>';
            m += '<td style="background:#FFFFFF;width:350px" valign="top" colspan="3" rowspan="12"><div  style="overflow:scroll; height:265px;width:350px" id="recentScouts"></div></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Militiamen:</b></td><td style="background:none;border-bottom:none"><label id="unit2">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Trebuchets:</b></td><td style="background:none;border-bottom:none"><label id="def2">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Scouts:</b></td><td style="background:none;border-bottom:none"><label id="unit3">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Spiked Barriers:</b></td><td style="background:none;border-bottom:none"><label id="def3">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Pikemen:</b></td><td style="background:none;border-bottom:none"><label id="unit4">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Caltrops:</b></td><td style="background:none;border-bottom:none"><label id="def4">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Swordsmen:</b></td><td style="background:none;border-bottom:none"><label id="unit5">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Traps:</b></td><td style="background:none;border-bottom:none"><label id="def5">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Archers:</b></td><td style="background:none;border-bottom:none"><label id="unit6">0</label></td>';
            m += '<th style="font-size:14px" colspan="2" rowspan="2" valign="bottom">Resources</th></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Cavalry:</b></td><td style="background:none;border-bottom:none"><label id="unit7">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Heavy Cavalry:</b></td><td style="background:none;border-bottom:none"><label id="unit8">0</label></td>';
            m += '<td width="45" style="background:none;border-bottom:none"><b>Food:</b></td><td style="background:none;border-bottom:none" width="70"><label id="res1">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Supply Wagons:</b></td><td style="background:none;border-bottom:none"><label id="unit9">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Wood:</b></td><td style="background:none;border-bottom:none"><label id="res2">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Ballistae:</b></td><td style="background:none;border-bottom:none"><label id="unit10">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Stone:</b></td><td style="background:none;border-bottom:none"><label id="res3">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Battering Rams:</b></td><td style="background:none;border-bottom:none"><label id="unit11">0</label></td>';
            m += '<td style="background:none;border-bottom:none"><b>Ore:</b></td><td style="background:none;border-bottom:none"><label id="res4">0</label></td></tr>';
            m += '<tr><td style="background:none;border-bottom:none"><b>Catapults:</b></td><td style="background:none;border-bottom:none"><label id="unit12">0</label></td></tr></table>';
			
			m += '<table><th style="font-size:14px" colspan="3">Attacks</th><tr><td style="background:none;border-bottom:none">';
			m += '<table><tr><td style="background:none;border-bottom:none"></td><td style="background:none;border-bottom:none"></td><td style="background:none;border-bottom:none" align="center">Wave 1</td><td style="background:none;border-bottom:none" align="center">Wave 2</td><td style="background:none;border-bottom:none" align="center">Wave 3</td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Supply Troops" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_1_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit1lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit1wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit1wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit1wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Militiamen" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_2_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit2lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit2wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit2wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit2wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Scouts" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_3_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit3lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit3wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit3wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit3wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Pikemen" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_4_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit4lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit4wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit4wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit4wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Swordsmen" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_5_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit5lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit5wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="calcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit5wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit5wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Archers" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_6_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit6lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit6wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit6wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit6wv3" size="6" /></td></tr>';
			m += '</table></td><td style="background:none;border-bottom:none">';
			m += '<table><tr><td style="background:none;border-bottom:none"></td><td style="background:none;border-bottom:none"></td><td style="background:none;border-bottom:none" align="center">Wave 1</td><td style="background:none;border-bottom:none" align="center">Wave 2</td><td style="background:none;border-bottom:none" align="center">Wave 3</td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Cavalry" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_7_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit7lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit7wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit7wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit7wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Heavy Cavalry" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_8_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit8lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit8wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit8wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit8wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Supply Wagons" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_9_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit9lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit9wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit9wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit9wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Ballistae" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_10_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit10lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit10wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit10wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit10wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Battering Rams" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_11_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit11lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit11wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit11wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit11wv3" size="6" /></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><img title="Catapults" height="25" src="http://cdn1.kingdomsofcamelot.com/fb/e2/src/img/units/unit_12_50.jpg?6545"></td><td style="background:none;border-bottom:none"><label id="unit12lbl">200,000</label></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit12wv1" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit12wv2" size="6" /></td><td style="background:none;border-bottom:none"><input onkeydown="btcalcTime()" style="background-color:#CCC" disabled="true" value="0" id="unit12wv3" size="6" /></td></tr>';
			m += '</table></td>';
			m += '<td style="background:none;border-bottom:none"><table>';
			m += '<tr><td style="background:none;border-bottom:none">Waves Available: <b><label style="font-size:14px" id="attMarches">0</label></b></td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><input type="checkbox" id="wv1chk" onclick="waveEnable(1)"/>Wave 1 Enable</td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><input type="checkbox" id="wv2chk" onclick="waveEnable(2)"/>Wave 2 Enable</td></tr>';
			m += '<tr><td style="background:none;border-bottom:none"><input type="checkbox" id="wv3chk" onclick="waveEnable(3)"/>Wave 3 Enable</td></tr>';
			m += '<tr><td style="background:none;border-bottom:none" align="center"><input type="button" id="waveLaunch" value="Launch"/></td></tr>';
			m += '</table></td></tr>';
    		m += '<tr><td style="background:none;border-bottom:none" colspan="2"><table width="100%"><tr>';
    		m += '<td style="background:none;border-bottom:none"><b>Wave 1:</b>&nbsp;&nbsp;<label id="wv1time">2 m 30 s</label></td>';
    		m += '<td style="background:none;border-bottom:none"><b>Wave 2:</b>&nbsp;&nbsp;<label id="wv2time">2 m 30 s</label></td>';
    		m += '<td style="background:none;border-bottom:none"><b>Wave 3:</b>&nbsp;&nbsp;<label id="wv3time">2 m 30 s</label></td>';
			m +='</tr></table></td></tr></table>';
        t.cont.innerHTML = m;
        document.getElementById('sOn').addEventListener ('click', t.btgetLatestReport, false);
        new CdispCityPicker ('bbtloc1', document.getElementById('bbtloc1'), true, t.bteventLocChanged, 0).bindToXYboxes(document.getElementById('bcalcX'), document.getElementById('bcalcY'));
        t.bteventLocChanged(Cities.cities[0], Cities.cities[0].x, Cities.cities[0].y);
		document.getElementById('attMarches').innerHTML = t.btgetKnightIds(Cities.cities[0].id).length;
        },
   ...
   //work on btmarchTimeCalculator
   
    btgetLatestReport: function (){
        var t = Tabs.Battle;
        document.getElementById('recentScouts').innerHTML = '';
        var xCheck = parseInt(document.getElementById('xInput').value);
        var yCheck = parseInt(document.getElementById('yInput').value);
        var rptFound = false;
        for(i=1;i<11;i++){
            var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
              params.pageNo = i;
            ...
            new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/listReports.php" + unsafeWindow.g_ajaxsuffix, {
              ...
              parameters: params,
              onSuccess: function (rslt) {
                t.createScoutList(rslt.arReports, rslt.arPlayerNames, rslt.arAllianceNames);
              },
              onFailure: function (rslt) {
              },
            }, false);
    ...
    createScoutList : function (ar,plrs,ally){
        var t = Tabs.Battle;
        if(t.lastRec == 1) t.scoutList = '<table>';
        var rptkeys = unsafeWindow.Object.keys(ar);
           for (var i = 0; i < rptkeys.length; i++) {
               var rpt = ar[rptkeys[i]];
              rpt.reportId = parseInt(rpt.reportId);
            rpt.marchType = parseInt(rpt.marchType);
            rpt.side0XCoord = parseInt(rpt.side0XCoord);
            rpt.side0YCoord = parseInt(rpt.side0YCoord);
            if(rpt.marchType == 3){
                var temp = Array(rpt.reportId, rpt.side0XCoord, rpt.side0YCoord, plrs['p'+parseInt(rpt.side0PlayerId)], plrs['p'+parseInt(rpt.side1PlayerId)], rpt.reportUnixTime, ally['a'+ parseInt(rpt.side0AllianceId)],rpt.side0AllianceId);
				if(ally['a'+ parseInt(rpt.side0AllianceId)] != 'SOC Prime'){
	                t.scoutList1.push(temp);
    	            t.scoutListSort.push(rpt.reportId);
				}
            }
        }
        t.lastRec++;
        if(t.lastRec == 11){
            t.scoutList = '<table><tr><td><b>Coords</b></td><td><b>Target</b></td><td><b>Alliance</b></td><td><b>Attacker</b></td></tr>';
            t.scoutListSort.sort();
            for(i=t.scoutListSort.length; i>0; i--){
                for(j=0; j<t.scoutList1.length; j++){
                    if(t.scoutList1[j][0] == t.scoutListSort[i]){
						if(typeof t.scoutList1[j][6] == "undefined") t.scoutList1[j][6] = 'None';
                        t.scoutList += '<tr><td nowrap="true"><a onclick="btactivateCoords('+ t.scoutList1[j][0]+ ','+ t.scoutList1[j][1]+ ','+ t.scoutList1[j][2]+ ',\''+ t.scoutList1[j][3]+ '\',\''+ t.scoutList1[j][4]+ '\','+ t.scoutList1[j][5]+')">' + t.scoutList1[j][1] +', '+t.scoutList1[j][2]+'</a></td><td nowrap="true">'+t.scoutList1[j][3]+'</td><td nowrap="true">';
						if(t.scoutList1[j][6] != 'None'){
							t.scoutList += '<a onclick="btbattleGetAlliance('+ t.scoutList1[j][7]+')">'+t.scoutList1[j][6].substr(0,15)+'</a>';
						}
						else{
							t.scoutList += t.scoutList1[j][6].substr(0,15);
						}
						t.scoutList +='</td><td nowrap="true">'+t.scoutList1[j][4].substr(0,10)+'</td></tr>';
                    }
                    if(!t.rptFound && t.scoutList1[j][1] == parseInt(document.getElementById('xInput').value) && t.scoutList1[j][2] == parseInt(document.getElementById('yInput').value)){
                        t.rptFound = true;
                        t.btgetMyReport(t.scoutList1[j][0],t.scoutList1[j][4], t.scoutList1[j][3], t.scoutList1[j][5]);
                    }
                }
            }
            t.scoutList += '</table>';
            document.getElementById('recentScouts').innerHTML += t.scoutList;
            t.lastRec = 1;
            t.scoutList1.length = 0;
            t.scoutListSort.length = 0;
        }
    }, 
	
    btgetMyReport : function (rpt_id, attName, defName, rptTime){
        var t = Tabs.Battle;
        var params = unsafeWindow.Object.clone(unsafeWindow.g_ajaxparams);
        params.rid = rpt_id;
        params.side = 1;
        new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/fetchReport.php" + unsafeWindow.g_ajaxsuffix,
            {
                method: "post",
                   parameters: params,
                onSuccess: function (rslt) {
                    updateLatest(rpt_id, rslt.unts, rslt.frt, rslt.rsc, attName, defName, rptTime, rslt.winner);
                },
                onFailure: function () {
                  if (notify)
                    notify ('AJAX ERROR');
        ...
        function updateLatest(rptId, units, defs, res, attacker, defender, rptTime, winner){
	        document.getElementById('searchMessage').innerHTML = rptId;

            if(attacker != '') document.getElementById('attLabel').innerHTML = attacker;
            else document.getElementById('attLabel').innerHTML = '';
            if(defender != '') document.getElementById('defLabel').innerHTML = defender;
            else document.getElementById('defLabel').innerHTML = '';
            if(rptTime != '') document.getElementById('timeLabel').innerHTML = unsafeWindow.formatDateByUnixTime(rptTime);
            else document.getElementById('timeLabel').innerHTML = '';
           
            if(units){
                if(units['u1']) document.getElementById('unit1').innerHTML = btaddCommasInt(units['u1']);
                else document.getElementById('unit1').innerHTML = 0;
                if(units['u2']) document.getElementById('unit2').innerHTML = btaddCommasInt(units['u2']);
                else document.getElementById('unit2').innerHTML = 0;
                if(units['u3']) document.getElementById('unit3').innerHTML = btaddCommasInt(units['u3']);
                else document.getElementById('unit3').innerHTML = 0;
                if(units['u4']) document.getElementById('unit4').innerHTML = btaddCommasInt(units['u4']);
                else document.getElementById('unit4').innerHTML = 0;
                if(units['u5']) document.getElementById('unit5').innerHTML = btaddCommasInt(units['u5']);
                else document.getElementById('unit5').innerHTML = 0;
                if(units['u6']) document.getElementById('unit6').innerHTML = btaddCommasInt(units['u6']);
                else document.getElementById('unit6').innerHTML = 0;
                if(units['u7']) document.getElementById('unit7').innerHTML = btaddCommasInt(units['u7']);
                else document.getElementById('unit7').innerHTML = 0;
                if(units['u8']) document.getElementById('unit8').innerHTML = btaddCommasInt(units['u8']);
                else document.getElementById('unit8').innerHTML = 0;
                if(units['u9']) document.getElementById('unit9').innerHTML = btaddCommasInt(units['u9']);
                else document.getElementById('unit9').innerHTML = 0;
                if(units['u10']) document.getElementById('unit10').innerHTML = btaddCommasInt(units['u10']);
                else document.getElementById('unit10').innerHTML = 0;
                if(units['u11']) document.getElementById('unit11').innerHTML = btaddCommasInt(units['u11']);
                else document.getElementById('unit11').innerHTML = 0;
                if(units['u12']) document.getElementById('unit12').innerHTML = btaddCommasInt(units['u12']);
                else document.getElementById('unit12').innerHTML = 0;
            }
            else{
                for(i=1; i<13; i++) document.getElementById('unit'+i).innerHTML = 0;
            }
           
            if(defs){
                if(defs['f53']) document.getElementById('def1').innerHTML = btaddCommasInt(defs['f53']);
                else document.getElementById('def1').innerHTML = 0;
                if(defs['f55']) document.getElementById('def2').innerHTML = btaddCommasInt(defs['f55']);
                else document.getElementById('def2').innerHTML = 0;
                if(defs['f62']) document.getElementById('def3').innerHTML = btaddCommasInt(defs['f62']);
                else document.getElementById('def3').innerHTML = 0;
                if(defs['f61']) document.getElementById('def4').innerHTML = btaddCommasInt(defs['f61']);
                else document.getElementById('def4').innerHTML = 0;
                if(defs['f60']) document.getElementById('def5').innerHTML = btaddCommasInt(defs['f60']);
                else document.getElementById('def5').innerHTML = 0;
            }
            else{
                for(i=1; i<6; i++) document.getElementById('def'+i).innerHTML = 0;
            }
            if(res){
                if(res['r1']) document.getElementById('res1').innerHTML = btaddCommasInt(parseInt(res['r1']));
                else document.getElementById('res1').innerHTML = 0;
                if(res['r2']) document.getElementById('res2').innerHTML = btaddCommasInt(parseInt(res['r2']));
                else document.getElementById('res2').innerHTML = 0;
                if(res['r3']) document.getElementById('res3').innerHTML = btaddCommasInt(parseInt(res['r3']));
                else document.getElementById('res3').innerHTML = 0;
                if(res['r4']) document.getElementById('res4').innerHTML = btaddCommasInt(parseInt(res['r4']));
                else document.getElementById('res4').innerHTML = 0;
            }
            else{
                for(i=1; i<5; i++) document.getElementById('res'+i).innerHTML = 0;
            }
			if(winner == 1) document.getElementById('scoutLabel').innerHTML = 'Yes';
			else document.getElementById('scoutLabel').innerHTML = 'No';
            t.bteventLocChanged(Cities.cities[0], Cities.cities[0].x, Cities.cities[0].y);             
        }
    },
   
    bteventLocChanged : function (city, x, y){
        var t = Tabs.Battle;		
        var x1 = parseInt(document.getElementById('xInput').value);
        var x2 = parseInt(document.getElementById('bcalcX').value);
        if (isNaN(x2))
              return;
        var y1 = parseInt(document.getElementById('yInput').value);
        var y2 = parseInt(document.getElementById('bcalcY').value);
        var m = 'Distance:  &nbsp;<B>'+ btdistance (x1, y1, x2, y2).toFixed(2) +'</b>';
		var cityId = 0;
        document.getElementById('bbtdistout').innerHTML = m;
        for(i=0; i<Cities.numCities; i++){
            if(document.getElementById('bbtloc1_'+i).className.toString() == 'btcastleBut btcastleButSel'){
                  cityId = Cities.cities[i].id;
            }
        }
		for (r=1; r<13; r++){
			document.getElementById('unit'+r+'lbl').innerHTML = btaddCommasInt(parseInt(Seed.units['city'+cityId]['unt'+r]));
		}
		document.getElementById('attMarches').innerHTML = t.btgetKnightIds(cityId).length;
		unsafeWindow.btcalcTime();
      },
	  
	btmarchTimeCalculator : function(troops){
		var unitSpeed = {unt1:180,unt2:200,unt3:3000,unt4:300,unt5:275,unt6:250,unt7:1000,unt8:750,unt9:150,unt10:100,unt11:120,unt12:80};
		for(i=0; i<Cities.numCities; i++){
			if(document.getElementById('bbtloc1_'+i).className.toString() == 'btcastleBut btcastleButSel'){
        		var cityX = Seed.cities[i][2];
				var cityY = Seed.cities[i][3];
	       	}
   		}
		var speed = 99999;
		var total_troops = 0;
		for (var troop_type in troops) {
			...
			var troop_speed = parseInt(unitSpeed["unt" + troop_type]) * (1 + 0.1 * parseInt(Seed.tech.tch11));
			if (troop_type > 6) {
		...
		var x = Math.abs(parseInt(cityX) - parseInt(document.getElementById('xInput').value));
		if (x > 375) {
			x = 750 - x
		}
		var y = Math.abs(parseInt(cityY) - parseInt(document.getElementById('yInput').value));
		if (y > 375) {
			y = 750 - y
		}
...
unsafeWindow.launchScout = function (){
	var scouts = parseInt(document.getElementById('scoutQtyB').value);
	var cityId = 0;
	for(i=0; i<Cities.numCities; i++){
    	if(document.getElementById('bbtloc1_'+i).className.toString() == 'btcastleBut btcastleButSel'){
        	cityId = Cities.cities[i].id;
	...
	for(j=0; j<33; j++){
      	if(b['pos'+j][0] == 12)
    	marchMax = (parseInt(b['pos'+j][1])*10000);
   	}
	if(parseInt(scouts) < 1){
		alert("You must enter a value for scouts > 0");
		return;
	}
	else if(parseInt(scouts) > marchMax){
		alert("Too many scouts selected for Rally Point level");
		return;
	}
	else if(parseInt(scouts) > parseInt(Seed.units['city'+cityId]['unt3'])){
		alert('You do not have enough scouts');
		return;
	}
	...
	params.r3 = 0;
	params.r4 = 0;
	params.xcoord = document.getElementById('xInput').value;
	params.ycoord = document.getElementById('yInput').value;
	params.u3 = parseInt(scouts);
	params.type = 3;
	for(i=0; i<Cities.numCities; i++){
    	if(document.getElementById('bbtloc1_'+i).className.toString() == 'btcastleBut btcastleButSel'){
        	params.cid = Cities.cities[i].id;
       		cityName = Cities.cities[i].name;
		}
	}
	params.kid = getKnightId(params.cid);
	if(params.kid != 0){
		if(window.confirm('Send scout to '+params.xcoord+','+params.ycoord+' from '+cityName+'?')){
			new MyAjaxRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, {
				method: "post",
				parameters: params,
				onSuccess: function (rslt) {
					var timediff = parseInt(rslt.eta) - parseInt(rslt.initTS);
					var ut = unixtime();
					attach_addoutgoingmarch(rslt.marchId, rslt.marchUnixTime, ut + timediff, params.xcoord, params.ycoord, params.kid, rslt.tileId, rslt.tileType, rslt.tileLevel, params.cid);
					alert('Scout sent');
				},
                      onFailure: function (rslt) {
                      },
		...
		alert('No night available in selected city.  Please selected another city and try again.');
	}
       
	function getKnightId(cityId){
		var did = {};
		for (var i=0; i<knightRoles.length; i++){
			var leader = Seed.leaders['city'+cityId][knightRoles[i][1]+'KnightId'];
			if (leader == 0){}
	  	...
	  	for (k in Seed.knights['city'+cityId]){
			if (!did[k]){
				if(Seed.knights['city'+cityId][k].knightStatus != 10)
					return Seed.knights['city'+cityId][k].knightId;
                }
		}
		return 0;
	}
...
Tabs.Test = {
  ...
  init : function (div){
    ...
    var m = '<TABLE><TR><TD align=right>Scout: </td><TD><INPUT type=checkbox id=fakeIsScout></td></tr>\
        ...
        <INPUT id=btReloadKOC type=submit value="Reload KOC" \><BR>\
        <INPUT id=btTestSomething type=submit value="A Test" \>\
    ...
    document.getElementById('btReloadKOC').addEventListener ('click', t.reloadKOC, false);
    document.getElementById('btTestSomething').addEventListener ('click', t.testSomething, false);
...
//uwMYmodal_alliance_report_view("6149415",1,51,8,13459797,"Anngrethe","F","Dudious","M",2,451,151,1299907771,0,267,433);
  ...
  reloadKOC : function (){
    ...
    var t = '<FORM target="_top" action="'+ goto +'" method=post><INPUT id=xxbtButReload type=submit value=RELOAD><INPUT type=hidden name=s value="'+ getServerId() +'"</form>';
    var e = document.createElement ('div');
    e.innerHTML = t;
    document.body.appendChild (e);
    setTimeout (function (){document.getElementById('xxbtButReload').click();}, 0);
  },
  ...
  init : function (div){
    ...
    try {     
      m = '<TABLE class=btTab>\
        ...
        <TR><TD><INPUT id=btAllowWinMove type=checkbox /></td><TD>Enable window drag (move window by dragging top bar with mouse)</td></tr>\
        <TR><TD><INPUT id=btHideOnGoto type=checkbox /></td><TD>Hide window when clicking on map coordinates. </td></tr>\
      ...
      t.togOpt ('btHideOnGoto', 'hideOnGoto');
      t.togOpt ('btAllowWinMove', 'btWinDrag', mainPop.setEnableDrag);
...
function getCityMight (cityId){
    var b = Seed.buildings['city'+cityId];
    var ret = [];
    for(i=0; i<=20; i++) ret[i]=0;
    var curPos = 0;
    for(j = 0; j<=19; j++){
        for (var i=0; i<141; i++){
            if (b['pos'+i] && b['pos'+i][0] == j){
                if(j == 0 || j == 19) ret[j] += Math.pow(2,(parseInt(b['pos'+i][1])-1))*50;
                if(j == 1 || j == 2 || j == 3 || j == 4) ret[j] += Math.pow(2,(parseInt(b['pos'+i][1])-1))*5;
                if(j == 5 || j == 13) ret[j] += Math.pow(2,(parseInt(b['pos'+i][1])-1))*10;
                if(j == 6 || j == 7 || j == 11 || j == 12) ret[j] += Math.pow(2,(parseInt(b['pos'+i][1])-1))*30;
                if(j == 8 || j == 9 || j == 10 || j == 14 || j == 15 || j == 16 || j == 17 || j == 18) ret[j] += Math.pow(2,(parseInt(b['pos'+i][1])-1))*20;
            }
        }
        ret[20] += ret[j];
    }
...
var tabManager = {
  ...
  init : function (mainDiv){
    ...
    var m = '<TABLE cellspacing=0 class=btMainTab><TR>';
    for (var i=0; i<sorter.length; i++)
//      m += '<TD class=spacer></td><TD class=notSel id=bttc'+ sorter[i][1].name +' ><A><SPAN>'+ sorter[i][1].label +'</span></a></td>';
//    m += '<TD class=spacer width=90% align=right>'+ Version +'&nbsp;</td></tr></table>';
    ...
    for (k in t.tabList) {
      ...
      document.getElementById('bttc'+ k).addEventListener('click', this.e_clickedTab, false);
    ...
    t.setTabStyle (document.getElementById ('bttc'+ t.currentTab.name), true);
  ...
  e_clickedTab : function (e){
    ...
    if (t.currentTab.name != newTab.name){
      t.setTabStyle (document.getElementById ('bttc'+ t.currentTab.name), false);
      t.setTabStyle (document.getElementById ('bttc'+ newTab.name), true);
      t.currentTab.obj.hide ();
      t.currentTab.div.style.display = 'none';
      t.currentTab = newTab;
...
function CdispCityPicker (id, span, dispName, notify, selbut){
  function CcityButHandler (t){
    ...
    function clickedCityBut (e){
      if (that.selected != null)
        that.selected.className = "btcastleBut btcastleButNon";
      that.city = Cities.cities[e.target.id.substr(that.prefixLen)];
      if (that.dispName)
        document.getElementById(that.id+'cname').innerHTML = that.city.name;
      e.target.className = "btcastleBut btcastleButSel";
  ...
  for (var i=0; i<Cities.cities.length; i++)
    m += '<INPUT class="btcastleBut btcastleButNon" id="'+ id +'_'+ i +'" value="'+ (i+1) +'" type=submit \>';
  if (dispName)
    m += ' &nbsp; <SPAN style="display:inline-block; width:170px; font-weight:bold;" id='+ id +'cname' +'></span>';
  span.innerHTML = m;
...
function CdialogCancelContinue (msg, canNotify, contNotify, centerElement){
  var pop = new CPopup ('btcancont', 0, 0, 400,200, true, canNotify);
  if (centerElement)
    pop.centerMe(centerElement);
  else
    pop.centerMe(document.body);
  pop.getTopDiv().innerHTML = '<CENTER>KOC Power Bot</center>';
  pop.getMainDiv().innerHTML = '<TABLE class=btTab align=center style="height: 100%"><TR align=center height=90%><TD>'+ msg +'</td></tr>\
      <TR align=center><TD><INPUT id=btcccancel type=submit value="CANCEL" \> &nbsp; &nbsp; <INPUT id=btcccontin type=submit value="CONTINUE" \></td></tr></table>';
  document.getElementById('btcccancel').addEventListener ('click', function (){pop.show(false); if (canNotify) canNotify();}, false);
  document.getElementById('btcccontin').addEventListener ('click', function (){pop.show(false); if (contNotify) contNotify();}, false);
  pop.show(true);
}
...
function dialogRetry (errMsg, seconds, onRetry, onCancel){
  ...
  var pop = new CPopup ('btretry', 0, 0, 400,200, true);
...
unsafeWindow.btGotoMapHide = function (x, y){
  try {
  ...
  unsafeWindow.btGotoMap (x, y); 
}



unsafeWindow.btGotoMap = function (x, y){
...
function makeButton20 (label){
  var a = document.createElement('a');
  a.className = "button20 btButton20";
...
function strButton20 (label, tags){
  if (tags == null)
    tags = '';
  return ('<TABLE class=btNoPad><TR><TD><A class="button20 btButton20" '+ tags +'><SPAN>'+ label +'</span></a></td></tr></table>' );
}

function strButton14 (label, tags){
  if (tags == null)
    tags = '';
  return ('<A class="button14 btButton20" '+ tags +'><SPAN>'+ label +'</span></a>' );
}

function btcityStatusString (cs){
  if (cs==4)
    return 'V';
  if (cs==3)
    return 'T';
  ...
  write : function (msg){
    ...
    if (t.reverse){
      ...
      t.eOut.appendChild(document.createElement('hr'));
		...
		function attach_addoutgoingmarch(marchId, marchTime, unixTime, x, y, kid, tileId, tileType, tileLevel, cid) {
			var f = Seed.units["city" + cid];
			f["unt2"] = parseInt(f["unt2"]) - 1;
	
			if (parseInt(kid) != 0) {
				Seed.knights["city" + cid]["knt" + kid].knightStatus = 10
			}
	
			attack_addqueue(marchId, marchTime, unixTime, x, y, kid, tileId, tileType, tileLevel, cid);
		}
		
		function attack_addqueue(marchId, marchTime, unixTime, x, y, kid, tileId, tileType, tileLevel, cid) {
			var b = new Object();
			b.destinationUnixTime = parseInt(unixTime);
			b.marchUnixTime = parseInt(marchTime);
			...
			if (tileId) {
				b.toTileId = tileId;
				b.toTileType = tileType;
				b.toTileLevel = tileLevel
			}
			for (var h = 1; h < 5; h++) {
				b["resource" + h] = 0
			}
			...
			if (isArray(Seed.queue_atkp["city" + cid])) {
				Seed.queue_atkp["city" + cid] = new Object()
			}
			Seed.queue_atkp["city" + cid]["m" + marchId] = b
		}		
...
unsafeWindow.closeTools = function(){
  mainPop.show (false);
}
